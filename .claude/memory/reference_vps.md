...
curl -s -X POST "https://auth.teezfpo.com/realms/master/protocol/openid-connect/token" \
  ...
  -d "username=corey" \
  -d "password=Alchupiz12?" \
  -d "grant_type=password"
```

### Common API calls
```bash
TOKEN=$(curl -s -X POST "https://auth.teezfpo.com/realms/master/protocol/openid-connect/token" -d "client_id=admin-cli" -d "username=corey" -d "password=Alchupiz12?" -d "grant_type=password" | jq -r '.access_token')
