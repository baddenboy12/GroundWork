...
const CreateSiteDialog = lazy(() => import("./CreateSiteDialog.tsx"));
const EditSiteDialog = lazy(() => import("./EditSiteDialog.tsx"));
...
export default function SitePopout({
  selectedSiteId,
  onSelectSite,
  onSiteDeleted,
}: Props) {
  ...
  const activeVotesBySite = useMemo(() => {
    if (!activeVotesRaw)
      return new Map<
        string,
        { voteId: Id<"siteDeleteVotes">; approvedCount: number }
      >();
  ...
  const pendingNewSiteNames = useMemo(() => {
    const existingNames = new Set(
      (sites ?? []).map((s) => s.name.toLowerCase()),
    );
  ...
  const atSiteLimit =
    config.maxSites !== null && ownSiteCount >= config.maxSites;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{
          background:
            "linear-gradient(160deg, hsl(30 14% 18%) 0%, hsl(30 12% 10%) 50%, hsl(30 14% 14%) 100%)",
        }}
      >
      ...
      <Popover>
        ...
        <PopoverContent
          side="bottom"
          align="start"
          className="w-72 p-4 space-y-3 text-sm"
        >
          <p className="font-semibold text-foreground">How Sites Work</p>
          <div className="space-y-1.5">
            <p className="text-xs font-medium text-foreground">
              Site name = grouping key
            </p>
            <p className="text-xs text-muted-foreground leading-relaxed">
              All log entries with the same site name are grouped under one
              site. Renaming a site updates every log entry under it instantly.
            </p>
          </div>
          <div className="space-y-1.5">
            <p className="text-xs font-medium text-foreground">
            ...
            <ul className="text-xs text-muted-foreground space-y-1.5 leading-relaxed">
              {[
                [
                  ...
                  "Type a new site name in the log dialog and the site is created automatically.",
                ],
                [
                  "Autocomplete selector",
                  "The site field suggests existing sites as you type to prevent duplicates.",
                ],
                [
                  "Fuzzy-match warning",
                  "If a name is close to an existing site a 'Did you mean?' hint appears.",
                ],
                [
                  "GPS & map location",
                  "Each site and log entry can store GPS coordinates from a live map.",
                ],
                [
                  "Bulk rename via Edit Site",
                  "Renaming a site updates all its log entries at once.",
                ],
              ].map(([title, desc]) => (
          ...
          >
            ...
            <div
              ...
              style={{
                ...
                borderRadius: "24px",
                background:
                  "linear-gradient(to bottom, hsl(30 14% 15%) 0%, hsl(30 12% 10%) 80px)",
                border: "1px solid var(--border)",
              }}
            >
              {/* Panel header */}
              ...
              <div
                ref={listRef}
                className="py-2 max-h-[65vh] overflow-y-auto px-1"
              >
                {sites === undefined ? (
                  ...
                  <div className="px-5 py-6 text-center">
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      No sites yet. Sites are created automatically when you add
                    ...
                    {sites.map((site, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <DropdownMenu
                            onOpenChange={(isOpen) =>
                              handleMenuOpenChange(site._id, isOpen)
                            }
                          >
                            <DropdownMenuTrigger
                              asChild
                            ...
                            <DropdownMenuContent
                              align="end"
                              className="w-52 p-3 rounded-2xl"
                              style={{
                                backgroundColor: "hsl(30 12% 8%)",
                                border: "1px solid var(--border)",
                              }}
                            >
                              {/* Edit — owner only */}
                              <DropdownMenuItem
                                className={cn(
                                  "py-4 text-base cursor-pointer rounded-xl",
                                  (!isOnline || !site.isOwner) && "opacity-50",
                                )}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (!isOnline) {
                                  ...
                                  if (!site.isOwner) {
                                    toast.error(
                                      "Only the site owner can edit it",
                                    );
                                    return;
                                  }
                                  setEditSite(site);
                              ...
                              >
                                <Settings className="w-4 h-4 mr-2.5" /> Edit
                                site
                              </DropdownMenuItem>
                              ...
                              >
                                {isTeamSite ? (
                                  <>
                                    ...
                                    {activeVote
                                      ? `Vote in progress (${activeVote.approvedCount})`
                                      : "Propose deletion"}
                                  </>
                                ) : (
                                  <>
