...
type Props = {
  ...
  /** Incremented by the parent when native back should request closing this dialog */
...
export default function CreateLogDialog({
  ...
  const deleteOrphanedPhotos = useAction(
    api.r2.storageActions.deleteOrphanedPhotos,
  );
  ...
  const hasDraft =
    ...
    ((location.trim().length > 0 || coords !== null) && !locationFromSite);
  ...
  useEffect(() => {
    ...
    if (closeRequestId !== lastCloseRequestIdRef.current) {
      lastCloseRequestIdRef.current = closeRequestId;
      requestClose();
    }
  }, [closeRequestId, open, requestClose]);
            ...
            >
              ...
              {!isOnline && (
                <div className="flex items-center gap-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 px-4 py-3 text-sm text-amber-700 dark:text-amber-400">
                  ...
                  <span>
                    You&apos;re offline. This entry and any photos will be saved
                        ...
                        {fuzzyMatches.length > 0 && (
                          <>
                            <p className="px-3 py-2 text-xs font-semibold uppercase tracking-wider text-amber-600 dark:text-amber-400 bg-amber-50/60 dark:bg-amber-900/20 flex items-center gap-1.5">
                              <AlertTriangle className="w-3 h-3" /> Did you
                              mean?
                            </p>
                            {fuzzyMatches.map((s) => (
                              <button
                ...
                {siteName.trim() &&
                  ...
                  fuzzyMatches.length === 0 && (
                    <p className="text-sm text-muted-foreground flex items-center gap-1">
                      <Plus className="w-3.5 h-3.5 text-primary" />A new site
                      named &quot;{siteName.trim()}&quot; will be created
                    </p>
                  )}
              </motion.div>
              ...
              >
                <Label htmlFor="log-title" className="text-xl font-semibold">
                  Title *
                </Label>
                <Input
                  id="log-title"
        ...
        <AlertDialogContent className="rounded-3xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base">
              This will discard the information and photos currently in this new
              log.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-3">
            <AlertDialogCancel className="h-14 rounded-2xl text-lg">
              Keep editing
            </AlertDialogCancel>
            <AlertDialogAction
              className="h-14 rounded-2xl bg-destructive px-6 text-lg text-white hover:bg-destructive/90"
