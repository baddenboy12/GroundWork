...
export default function EditSiteDialog({ open, onClose, site }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent
        className="!max-w-none w-[90%] p-8 top-[8%] translate-y-0 overflow-y-auto max-h-[90vh] rounded-3xl [&>button]:w-16 [&>button]:h-16 [&>button>svg]:w-9 [&>button>svg]:h-9 [&>button]:active:scale-75 [&>button]:transition-transform" onOpenAutoFocus={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()}>
        <DialogHeader>
